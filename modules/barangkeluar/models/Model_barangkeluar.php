...
class Model_barangkeluar extends MY_Model {

    private $primary_key    = 'id_barangkeluar';
    private $table_name     = 'barangkeluar';
    public $field_search   = ['id_barang', 'tujuan', 'jumlah', 'barang.nama_barang', 'sumber.nama_sumber'];
    public $sort_option = ['id_barangkeluar', 'DESC'];
    ...
    public function count_all($q = null, $field = null)
    {
        ...
        if (empty($field)) {
            ...
            $where .= "(" . "barangkeluar.".$field . " LIKE '%" . $q . "%' )";
        }
    ...
    public function join_avaiable() {
        $this->db->join('barang', 'barang.id_barang = barangkeluar.id_barang', 'LEFT');
    ...
    public function export_excel($subject = 'file') {
        $result = $this->db->query("SELECT
            @NO := @NO + 1 AS 'no',
            barangkeluar.tanggal AS tanggal,
            `sumber`.`nama_sumber` AS nama_sumber,
            `barang`.`nama_barang` AS nama_barang,
            `barangkeluar`.`jumlah` AS jumlah_barang,
            `satuan`.`nama_satuan` AS satuan,
            `barangkeluar`.keterangan AS keterangan 
        FROM
            `barangkeluar`
            JOIN ( SELECT @NO := 0 ) AS no_urut
            LEFT JOIN `sumber` ON `sumber`.`id_sumber` = `barangkeluar`.tujuan
            LEFT JOIN `barang` ON `barang`.`id_barang` = `barangkeluar`.`id_barang`
            LEFT JOIN `satuan` ON `barang`.`satuan` = `satuan`.`id_satuan` 
        ORDER BY
            barangkeluar.tanggal ASC");
		...
		$this->excel->getActiveSheet()->setCellValue('A1', 'REKAP DATA BARANG KELUAR');
...
/* End of file Model_barangkeluar.php */
/* Location: ./application/models/Model_barangkeluar.php */