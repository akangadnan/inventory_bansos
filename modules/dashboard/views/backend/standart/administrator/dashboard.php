<section class="content">
	<div class="row">
		<div class="col-lg-3 col-xs-6">
			<div class="small-box bg-aqua">
				<div class="inner">
					<h3><?= count($barang);?> item</h3>
					<p>Data Barang</p>
				</div>
				<div class="icon">
					<i class="fa fa-database"></i>
				</div>
				<a href="<?= BASE_URL;?>administrator/barang" class="small-box-footer">More info <i class="fa fa-arrow-circle-right"></i></a>
			</div>
		</div>

		<div class="col-lg-3 col-xs-6">
			<div class="small-box bg-green">
				<div class="inner">
					<h3><?= count($masuk);?> Data</h3>
					<p>Barang Masuk</p>
				</div>
				<div class="icon">
					<i class="fa fa-database"></i>
				</div>
				<a href="<?= BASE_URL;?>administrator/barangmasuk" class="small-box-footer">More info <i class="fa fa-arrow-circle-right"></i></a>
			</div>
		</div>

		<div class="col-lg-3 col-xs-6">
			<div class="small-box bg-yellow">
				<div class="inner">
					<h3><?= count($keluar);?> Data</h3>
					<p>Barang Keluar</p>
				</div>
				<div class="icon">
					<i class="fa fa-database"></i>
				</div>
				<a href="<?= BASE_URL;?>administrator/barangkeluar" class="small-box-footer">More info <i class="fa fa-arrow-circle-right"></i></a>
			</div>
		</div>

		<div class="col-lg-3 col-xs-6">
			<div class="small-box bg-red">
				<div class="inner">
					<h3><?= count($retur);?> Data</h3>
					<p>Barang Retur</p>
				</div>
				<div class="icon">
					<i class="fa fa-database"></i>
				</div>
				<a href="<?= BASE_URL;?>administrator/retur" class="small-box-footer">More info <i class="fa fa-arrow-circle-right"></i></a>
			</div>
		</div>
	</div>
	<!-- <div class="row">
		<div class="col-md-12">
			<div class="box box-primary">
				<div class="box-header with-border">
					<h3 class="box-title">Data Barang</h3>
					<div class="box-tools pull-right">
						<a href="javascript:void(0);">Lihat Data Selengkapnya <i class="fa fa-external-link"></i></a>
					</div>
				</div>
				<div class="box-body">
					<div class="table-responsive">
						<table class="table no-margin">
							<thead>
								<tr>
									<th>Barang</th>
									<th>Jumlah</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td>OR9842</td>
									<td>Call of Duty IV</td>
								</tr>
								<tr>
									<td>OR1848</td>
									<td>Samsung Smart TV</td>
								</tr>
								<tr>
									<td>OR7429</td>
									<td>iPhone 6 Plus</td>
								</tr>
								<tr>
									<td>OR7429</td>
									<td>Samsung Smart TV</td>
								</tr>
								<tr>
									<td>OR1848</td>
									<td>Samsung Smart TV</td>
								</tr>
								<tr>
									<td>OR7429</td>
									<td>iPhone 6 Plus</td>
								</tr>
								<tr>
									<td>OR9842</td>
									<td>Call of Duty IV</td>
								</tr>
							</tbody>
						</table>
					</div>

				</div>
			</div>
		</div>
	</div>
	<div class="row">
		<div class="col-md-12">
			<div class="box box-primary">
				<div class="box-header with-border">
					<h3 class="box-title">Latest Orders</h3>
				</div>
				<div class="box-body">
					<div class="table-responsive">
						<table class="table no-margin">
						</table>
					</div>
				</div>
			</div>
		</div>
	</div> -->
</section>