...
<section class="content-header">
	<h1>
		<?= cclang('kecamatan') ?><small><?= cclang('list_all'); ?></small>
	</h1>
	<ol class="breadcrumb">
					...
					<div class="box box-widget widget-user-2">
						...
						<div class="widget-user-header ">
							<div class="row pull-right">
								<?php is_allowed('kecamatan_add', function(){?>
								<a class="btn btn-flat btn-success btn_add_new" id="btn_add_new"
									title="<?= cclang('add_new_button', [cclang('kecamatan')]); ?>  (Ctrl+a)"
									href="<?=  site_url('administrator/kecamatan/add'); ?>"><i
										class="fa fa-plus-square-o"></i>
									<?= cclang('add_new_button', [cclang('kecamatan')]); ?></a>
								<?php }) ?>
								<?php is_allowed('kecamatan_export', function(){?>
								<a class="btn btn-flat btn-success"
									title="<?= cclang('export'); ?> <?= cclang('kecamatan') ?> "
									href="<?= site_url('administrator/kecamatan/export?q='.$this->input->get('q').'&f='.$this->input->get('f')); ?>"><i
							...
							<!-- /.widget-user-image -->
							<h3 class="widget-user-username"><?= cclang('kecamatan') ?></h3>
							<h5 class="widget-user-desc"><?= cclang('list_all', [cclang('kecamatan')]); ?> <i
									class="label bg-yellow"><?= $kecamatan_counts; ?> <?= cclang('items'); ?></i></h5>
						</div>

						<form name="form_kecamatan" id="form_kecamatan"
							action="<?= base_url('administrator/kecamatan/index'); ?>">
								...
								<div class="col-md-8">
									...
									<div class="col-sm-3 padd-left-0 ">
										<select type="text" class="form-control chosen chosen-select" name="f"
											...
											<option <?= $this->input->get('f') == 'kecamatan_nama' ? 'selected' :''; ?>
												value="kecamatan_nama">Nama Kecamatan</option>
										</select>
									</div>
									...
									<div class="col-sm-1 padd-left-0 ">
										<a class="btn btn-default btn-flat" name="reset" id="reset" value="Apply"
											href="<?= base_url('administrator/kecamatan');?>"
								...
								<table class="table table-bordered table-striped dataTable">
									<thead>
										<tr class="">
											...
											<th data-field="kecamatan_nama" data-sort="1" data-primary-key="0">
												<?= cclang('kecamatan_nama') ?></th>
											<th>Action</th>
										</tr>
									</thead>
									<tbody id="tbody_kecamatan">
										...
										<tr>
											...
											<td><span
													class="list_group-kecamatan-nama"><?= _ent($kecamatan->kecamatan_nama); ?></span>
											</td>
											<td width="200">
