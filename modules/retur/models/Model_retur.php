...
class Model_retur extends MY_Model {
    ...
    public $field_search   = ['tanggal', 'id_barang', 'jumlah', 'keterangan', 'barang.nama_barang'];
    public $sort_option = ['id_retur', 'DESC'];
    ...
    public function join_avaiable() {
        ...
        $this->db->join('satuan', 'satuan.id_satuan = barang.id_barang', 'LEFT');
        
        $this->db->select('barang.nama_barang,satuan.*,retur.*,barang.nama_barang as barang_nama_barang,barang.nama_barang as nama_barang,satuan.nama_satuan as satuan_nama_satuan');
    ...
    public function export_excel($subject = 'file') {
        $result = $this->db->query("SELECT
            ...
            `retur`.`jumlah` AS jumlah_barang,
            `satuan`.`nama_satuan` AS satuan,
            `retur`.keterangan AS keterangan 
        FROM
            `retur`
            JOIN ( SELECT @NO := 0 ) AS no_urut
            LEFT JOIN `barang` ON `barang`.`id_barang` = `retur`.`id_barang`
		...
		$this->excel->getActiveSheet()->setCellValue('A1', 'REKAP DATA RETUR BARANG');
