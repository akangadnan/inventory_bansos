...
<section class="content-header">
	<h1>
		<?= cclang('retur') ?><small><?= cclang('list_all'); ?></small>
	</h1>
	<ol class="breadcrumb">
		<li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
		<li class="active"><?= cclang('retur') ?></li>
	</ol>
</section>
<!-- Main content -->
						...
						<div class="widget-user-header ">
							<div class="row pull-right">
								...
								<a class="btn btn-flat btn-success btn_add_new" id="btn_add_new"
									title="<?= cclang('add_new_button', [cclang('retur')]); ?>  (Ctrl+a)"
									href="<?=  site_url('administrator/retur/add'); ?>"><i
										class="fa fa-plus-square-o"></i>
									<?= cclang('add_new_button', [cclang('retur')]); ?></a>
								<?php }) ?>
								<?php is_allowed('retur_export', function(){?>
								<a class="btn btn-flat btn-success"
									title="<?= cclang('export'); ?> <?= cclang('retur') ?> "
									href="<?= site_url('administrator/retur/export?q='.$this->input->get('q').'&f='.$this->input->get('f')); ?>"><i
							...
							<!-- /.widget-user-image -->
							<h3 class="widget-user-username"><?= cclang('retur') ?></h3>
							<h5 class="widget-user-desc"><?= cclang('list_all', [cclang('retur')]); ?> <i
									class="label bg-yellow"><?= $retur_counts; ?> <?= cclang('items'); ?></i></h5>
						</div>
								...
								<div class="col-md-8">
									...
									<div class="col-sm-1 padd-left-0 ">
										<a class="btn btn-default btn-flat" name="reset" id="reset" value="Apply"
											href="<?= base_url('administrator/retur');?>"
								...
								<table class="table table-bordered table-striped dataTable">
									...
									<tbody id="tbody_retur">
										...
										<tr>
											...
											<td><span class="list_group-tanggal"><?= _ent($retur->tanggal); ?></span>
											</td>
											<td><?php echo $retur->barang_nama_barang;?>
											</td>

											<td><span class="list_group-jumlah"><?= _ent($retur->jumlah).' '._ent($retur->nama_satuan); ?></span></td>
											<td><span
													class="list_group-keterangan"><?= _ent($retur->keterangan); ?></span>
											</td>
											<td width="200">

												<?php is_allowed('retur_view', function() use ($retur){?>
												<a href="<?= site_url('administrator/retur/view/' . $retur->id_retur); ?>"
													class="label-default"><i class="fa fa-newspaper-o"></i>
													<?= cclang('view_button'); ?>
													<?php }) ?>
													<?php is_allowed('retur_update', function() use ($retur){?>
													<a href="<?= site_url('administrator/retur/edit/' . $retur->id_retur); ?>"
														class="label-default"><i class="fa fa-edit "></i>
														<?= cclang('update_button'); ?></a>
													<?php }) ?>
													<?php is_allowed('retur_delete', function() use ($retur){?>
													<a href="javascript:void(0);"
														data-href="<?= site_url('administrator/retur/delete/' . $retur->id_retur); ?>"
										...
										<tr>
											<td colspan="100">
												Retur Barang data is not available
											</td>
										</tr>
										<?php endif; ?>
...
<script>
	$(document).ready(function () {
		...
		$('#apply').click(function () {
			...
			if (bulk.val() == 'delete') {
				swal({
					...
					function (isConfirm) {
						if (isConfirm) {
							document.location.href = BASE_URL + '/administrator/retur/delete?' +
								serialize_bulk;
						}
					});
		...
		initSortable('retur', $('table.dataTable'));
	}); /*end doc ready*/
</script>