...
class Model_satuan extends MY_Model {
	private $primary_key 	= 'id_satuan';
	private $table_name 	= 'satuan';
	public $field_search 	= ['nama_satuan'];
	public $sort_option 	= ['id_satuan', 'DESC'];
	...
	public function export_excel($subject = 'file', $field_search = []) {
        $result = $this->db->query("SELECT @no:=@no+1 AS 'no', `satuan`.`nama_satuan` FROM `satuan`, ( SELECT @NO := 0 ) AS no_urut");
		...
		$this->excel->getActiveSheet()->setCellValue('A1', 'DATA SATUAN BARANG');
